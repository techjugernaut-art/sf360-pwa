...
import { ReadPaymentRequestRequirementsComponent } from '../dialog/read-payment-request-requirements/read-payment-request-requirements.component';
declare const swal;
...
export class AddPaymentRequestComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.businessInformationFormGroup = new FormGroup({
      ...
      // country: new FormControl('', [Validators.required]),
      business_state_of_residence: new FormControl(''),
      // business_city_of_residence: new FormControl('', [Validators.required]),
      business_address: new FormControl('', [Validators.required]),
      tin: new FormControl('', [Validators.required]),
      bvn: new FormControl('', [Validators.required]),
      description_and_nature_of_business: new FormControl(''),
      business_registration_number: new FormControl(''),
      reseller_code: new FormControl(''),
      secondary_contact_name: new FormControl(''),
      secondary_contact_picture: new FormControl(''),
    ...
    this.primaryContactInformationFormGroup = new FormGroup({
      ...
      primary_contact_email: new FormControl('', [Validators.email]),
      primary_contact_picture: new FormControl(''),
    ...
    this.companyDocumentationFormGroup = new FormGroup({
      ...
      certificate_of_registration: new FormControl('', [Validators.required]),
      cac_bn: new FormControl('', [Validators.required]),
      memorandum_and_articles_of_association: new FormControl('', [Validators.required]),
      form_cac_7: new FormControl('', [Validators.required]),
      form_cac_2: new FormControl('', [Validators.required]),
      shareholders_valid_id: new FormControl('', [Validators.required]),
      ...
      ngo_reg_certificate: new FormControl('', [Validators.required]),
      cooperative_registration_certificate: new FormControl('', [Validators.required]),
    ...
    this.shop_id.valueChanges.subscribe(value => {
      this.shopInfo = this.myShops.filter(data => {
        if (data.id === value) {
          const shopData = data;
          if (shopData !== null && shopData !== undefined) {
            // this.shop_id.setValue(shopData?.id)
            this.business_name.setValue(shopData?.business_name)
            this.business_type.setValue(shopData?.business_type)
            this.business_category.setValue(shopData?.business_category)
            this.business_address.setValue(shopData?.location)
            // this.secondary_contact_phone.setValue(shopData?.contact_number)
            // this.secondary_contact_email.setValue(shopData?.contact_email)
            this.primary_contact_name.setValue(shopData?.administration_name)
            // this.primary_contact_phone.setValue(shopData?.contact_number)
            // this.primary_contact_email.setValue(shopData?.contact_email)
            this.tin.setValue(shopData?.tin)
          }
        }
 ...
 */
  ...
  readRequirements() {
    this.dialog.open(ReadPaymentRequestRequirementsComponent, {
      data: {},
      position: { bottom: '30px', top: '10px' },
      disableClose: true
    });
  }
